...
Понимает vless://, trojan:// и ss:// — то, что отдают провайдеры с обфускацией.
И ссылка, и подписка содержат пароль: держите их с правами 600 и не коммитьте.
"""
...
# Узел внутри России бесполезен: фильтрация стоит на границе, и выход по ту же
# сторону от неё ничего не меняет. Такие отсеиваются всегда, а не по вкусу
HOMELAND = ("RU", "РФ")

# чем лучше маскировка, тем выше место в очереди: Reality прикидывается чужим
# сайтом, trojan прячется за обычным TLS, shadowsocks заметнее обоих
...
def choose(links: list[str], pick: str = "") -> str:
    ...
    suitable = [
        ...
        if not any(mark in name_of(link).upper().split() for mark in HOMELAND)
    ]
