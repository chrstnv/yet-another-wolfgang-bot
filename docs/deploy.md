...
## Шаг 0. Дотянется ли машина до Телеграма

**Это делается первым.** С февраля 2026 Телеграм в России замедляют, и с
российских хостингов `api.telegram.org` отвечает не всегда. Пока не измерено,
строить остальное бессмысленно.

На машине, подставив токен:

```bash
curl -sS -o /dev/null -w 'код %{http_code}, время %{time_total}\n' \
  https://api.telegram.org/bot$BOT_TOKEN/getMe

# час длинных опросов — так ходит сам бот
...
3. Сервисный аккаунт `wolfgang-ci` с ролью `container-registry.images.pusher`
   и федерация удостоверений с GitHub (`yc-actions/yc-cr-login` умеет обменивать
   токен GitHub на IAM-токен).
...
| Секрет | Что это |
|---|---|
| `YC_SA_ID` | идентификатор сервисного аккаунта `wolfgang-ci` |
